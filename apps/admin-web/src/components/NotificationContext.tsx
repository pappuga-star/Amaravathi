...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const showError = useCallback((err: any) => {
    ...
    } else if (rawMsg.includes('must be greater than 0') || rawMsg.includes('positive')) {
      msg = 'Quantity and price values must be greater than 0.';
    } else if (rawMsg.includes('exceeds available stock') || rawMsg.includes('Stock limit exceeded')) {
      msg = 'Requested quantity exceeds available stock.';
    } else if (rawMsg.includes('Duplicate ingredients') || rawMsg.includes('duplicate batch')) {
      msg = 'Duplicate ingredients found. Please resolve duplicate batch and ingredient selections.';
    }
  ...
  return (
    <NotificationContext.Provider value={{ showToast, showError, confirm }}>
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 w-full max-w-sm px-4 sm:px-0">
        {toasts.map((toast) => {
          const typeClasses = {
            success: 'bg-emerald-50 border-emerald-200 text-emerald-800 dark:bg-emerald-950 dark:border-emerald-900 dark:text-emerald-300',
            error: 'bg-rose-50 border-rose-200 text-rose-800 dark:bg-rose-950 dark:border-rose-900 dark:text-rose-300',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm">
          ...
          >
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <Button
                variant={confirmDialog.options.variant === 'danger' ? 'delete' : 'default'}
