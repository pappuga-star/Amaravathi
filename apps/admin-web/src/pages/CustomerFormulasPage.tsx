...
export const CustomerFormulasPage = ({ editingFormula, onCancelEdit }: CustomerFormulasPageProps) => {
  ...
  const updateLineItem = (index: number, fields: Partial<CustomerTeaFormulaLineItem>) => {
    const updated = [...lineItems];
    const current = { ...updated[index], ...fields } as CustomerTeaFormulaLineItem;
    ...
    if (fields.ingredientName !== undefined) {
      const selectedBatch = batches.find(b => b.batchCode === current.purchaseBatchCode);
      const batchItem = selectedBatch?.items.find(i => i.teaPowderType === fields.ingredientName);
      if (batchItem) {
        current.pricePerGram = batchItem.pricePerGram ?? (batchItem.ratePerKg / 1000);
        current.purchaseBatchLineItemId = (batchItem as any)._id?.toString() || '';
        current.ingredientCategory = batchItem.ingredientCategory || (
  ...
  const stockErrors = useMemo(() => {
    ...
    lineItems.forEach((item, idx) => {
      if (item.purchaseBatchCode && item.ingredientName && item.quantityInGrams > 0) {
        const selectedBatch = batches.find(b => b.batchCode === item.purchaseBatchCode);
        const batchItem = selectedBatch?.items.find(i => i.teaPowderType === item.ingredientName);
        if (batchItem) {
                ...
                <div className="grid gap-1.5 w-full md:w-1/2">
                  ...
                  <select
                    className={`h-10 rounded-lg border px-3 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 bg-white text-slate-800 ${
                      formErrors.customerId ? 'border-rose-500 ring-1 ring-rose-500 animate-pulse' : 'border-slate-300'
                    }`}
                    value={customerId}
                    onChange={(e) => {
                  ...
                  <table className="w-full text-left text-sm whitespace-nowrap min-w-[700px]">
                    ...
                    <tbody className="divide-y divide-slate-100 bg-white">
                      {lineItems.map((item, idx) => {
                        ...
                        const filteredIngredients = selectedBatch ? selectedBatch.items : [];
                        const isDuplicate = !!(
                        ...
                        return (
                          <tr key={idx} className="hover:bg-slate-50/50">
                            ...
                            <td className="px-3 py-2 text-xs font-semibold text-slate-700">
                              {lockedRows[idx] ? (
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    updateLineItem(idx, { purchaseBatchCode: e.target.value })
                                  }
                                  disabled={!canEdit}
                                >
                              ...
                              {lockedRows[idx] ? (
                                ...
                                <>
                                  <select
                                    ...
                                    onChange={(e) =>
                                      updateLineItem(idx, { ingredientName: e.target.value })
                                    }
                            ...
                            <td className="px-3 py-2 text-xs font-mono font-bold text-slate-500">
                              ₹{item.pricePerGram ? item.pricePerGram.toFixed(4) : '0.0000'}
                            </td>
                              ...
                              <div className="flex items-center justify-center gap-1.5">
                                {lockedRows[idx] ? (
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      setLockedRows({ ...lockedRows, [idx]: true });
                                    }}
                                  >
                                    <Check size={14} />
                                  </button>
