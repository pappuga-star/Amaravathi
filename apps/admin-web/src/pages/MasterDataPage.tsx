...
export const LeafCategoriesPage = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api<{ items: any[] }>(
        `${endpoints.leafCategories}?q=${encodeURIComponent(searchQuery)}&status=${statusFilter}&sortBy=name&sortOrder=asc`,
      ),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: any) =>
      editingId
        ? api(`${endpoints.leafCategories}/${editingId}`, {
            method: 'PUT',
            body: JSON.stringify(payload),
          })
        : api(endpoints.leafCategories, {
            method: 'POST',
            body: JSON.stringify(payload),
          }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      api(`${endpoints.leafCategories}/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [endpoints.leafCategories] });
  ...
  const restoreMutation = useMutation({
    mutationFn: (id: string) =>
      api(`${endpoints.leafCategories}/${id}/restore`, { method: 'POST' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [endpoints.leafCategories] });
    ...
    <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
      <Card className="h-max p-5 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col gap-4">
        <h3 className="text-lg font-bold text-slate-800">
          {editingId
            ? t('leafCategories.editTitle')
            : t('leafCategories.addTitle')}
        </h3>
        <form className="grid gap-4" onSubmit={handleSubmit}>
          <label className="grid gap-1.5 text-sm font-medium text-slate-700">
            ...
            <Input
              type="text"
              placeholder={t('leafCategories.placeholderName')}
              value={name}
          ...
          <label className="grid gap-1.5 text-sm font-medium text-slate-700">
            ...
            <textarea
              className="min-h-20 rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500"
              placeholder={t('leafCategories.placeholderDesc')}
              value={description}
      ...
      <Card className="min-w-0 p-5 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col gap-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-bold text-slate-800">
              {t('leafCategories.records')}
            </h3>
            <p className="text-xs text-slate-400">
              {t('leafCategories.totalBlended')}
            </p>
          </div>
          <div className="flex flex-wrap gap-2 items-center">
            <div className="flex rounded-lg border border-slate-200 p-0.5 bg-slate-50 text-xs font-semibold text-slate-600">
              {(['all', 'active', 'inactive', 'deleted'] as const).map(
                (status) => (
                  <button
                    key={status}
                    onClick={() => setStatusFilter(status)}
                    className={`px-3 py-1.5 rounded-md transition-all ${statusFilter === status ? 'bg-white text-emerald-700 shadow-sm' : 'hover:text-slate-900'}`}
                  >
                    {t(status)}
                  </button>
                ),
            ...
            <div className="relative flex items-center">
              ...
              />
              <Input
                className="pl-9 h-9 text-sm font-normal w-full sm:max-w-xs"
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                <tr>
                  ...
                  >
                    {t('leafCategories.loadingRecords')}
                  </td>
                </tr>
              ) : items.length === 0 ? (
                <tr>
                ...
                items.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm font-normal text-slate-500 max-w-xs truncate">
                      {item.description || '-'}
                    </td>
                        ...
                        {item.deletedAt ? (
                          ...
                          <>
                            ...
                            <button
                              ...
                              disabled={!canEdit}
                              className="inline-flex items-center justify-center h-8 w-8 rounded-lg bg-slate-50 text-slate-500"
                            >
                              <Edit3 size={14} />
                            </button>
                            <button
      ...
      {viewingCategory && (
        <ViewDetailsModal
          ...
          fields={[
            ...
            {
              label: t('status'),
              value: viewingCategory.deletedAt
                ? t('deleted')
                : viewingCategory.active
                  ? t('active')
                  : t('inactive'),
              type: 'badge',
              badgeColor: viewingCategory.deletedAt
                ? 'red'
                : viewingCategory.active
                  ? 'green'
                  : 'slate',
...
export const CuttingTypesPage = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    'all' | 'active' | 'inactive' | 'deleted'
  >('all');
  const [viewingCuttingType, setViewingCuttingType] = useState<any | null>(
    null,
  );
  ...
  const { data: leafData } = useQuery({
    ...
    queryFn: () =>
      api<{ items: any[] }>(
        `${endpoints.leafCategories}?status=active&limit=100`,
      ),
  });
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api<{ items: any[] }>(
        `${endpoints.cuttingTypes}?q=${encodeURIComponent(searchQuery)}&status=${statusFilter}&sortBy=name&sortOrder=asc`,
      ),
  });
  const items = data?.items ?? [];
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: any) =>
      editingId
        ? api(`${endpoints.cuttingTypes}/${editingId}`, {
            method: 'PUT',
            body: JSON.stringify(payload),
          })
        : api(endpoints.cuttingTypes, {
            method: 'POST',
            body: JSON.stringify(payload),
          }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      api(`${endpoints.cuttingTypes}/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [endpoints.cuttingTypes] });
  ...
  const restoreMutation = useMutation({
    mutationFn: (id: string) =>
      api(`${endpoints.cuttingTypes}/${id}/restore`, { method: 'POST' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [endpoints.cuttingTypes] });
  ...
  const handleEdit = (item: any) => {
    ...
    setLeafCategoryId(
      typeof item.leafCategoryId === 'object'
        ? item.leafCategoryId?.id || ''
        : item.leafCategoryId || '',
    );
    setDescription(item.description ?? '');
    setActive(item.active);
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
      <Card className="h-max p-5 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col gap-4">
        <h3 className="text-lg font-bold text-slate-800">
          {editingId ? t('cuttingTypes.editTitle') : t('cuttingTypes.addTitle')}
        </h3>
        <form className="grid gap-4" onSubmit={handleSubmit}>
          <label className="grid gap-1.5 text-sm font-medium text-slate-700">
            <span>{t('cuttingTypes.parentCategory')}</span>
            <select
              className="h-10 rounded-lg border border-slate-300 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 bg-white"
          ...
          <label className="grid gap-1.5 text-sm font-medium text-slate-700">
            ...
            <Input
              type="text"
              placeholder={t('cuttingTypes.placeholderName')}
              value={name}
              onChange={(e) => setName(e.target.value)}
              required
              disabled={!canEdit}
          ...
          <label className="grid gap-1.5 text-sm font-medium text-slate-700">
            ...
            <textarea
              className="min-h-20 rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500"
              placeholder={t('cuttingTypes.placeholderDesc')}
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              disabled={!canEdit}
            />
      ...
      <Card className="min-w-0 p-5 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col gap-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-bold text-slate-800">
              {t('cuttingTypes.records')}
            </h3>
            <p className="text-xs text-slate-400">
              {t('cuttingTypes.subtitle')}
            </p>
          </div>
          <div className="flex flex-wrap gap-2 items-center">
            <div className="flex rounded-lg border border-slate-200 p-0.5 bg-slate-50 text-xs font-semibold text-slate-600">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wide">
                  {t('leafCategories.title')}
                </th>
                <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wide">
                  {t('cuttingTypes.basePrice')}
                </th>
                <th className="px-4 py-3 font-semibold text-xs uppercase tracking-wide text-center">
                  {t('status')}
                </th>
                ...
                items.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-slate-600">
                      {typeof item.leafCategoryId === 'object'
                        ? item.leafCategoryId?.name
                        : item.leafCategoryId || '-'}
                    </td>
                    <td className="px-4 py-3 text-sm font-medium text-slate-600">
                      ₹{Number(item.basePrice).toFixed(2)}/kg
                    </td>
                    <td className="px-4 py-3 text-center">
                      {item.deletedAt ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-50 text-red-700">
        ...
        <ViewDetailsModal
          ...
          fields={[
            ...
            },
            {
              ...
              value:
                typeof viewingCuttingType.leafCategoryId === 'object'
                  ? viewingCuttingType.leafCategoryId?.name
                  : viewingCuttingType.leafCategoryId || '—',
            },
            {
            ...
            {
              label: t('status'),
              value: viewingCuttingType.deletedAt
                ? t('deleted')
                : viewingCuttingType.active
                  ? t('active')
                  : t('inactive'),
              type: 'badge',
              badgeColor: viewingCuttingType.deletedAt
                ? 'red'
                : viewingCuttingType.active
                  ? 'green'
                  : 'slate',
